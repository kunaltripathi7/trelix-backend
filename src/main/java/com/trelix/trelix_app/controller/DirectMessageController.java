...
public class DirectMessageController {
     ...
     * @param currentUser The CustomUserDetails of the authenticated user.
    ...
    public ResponseEntity<DirectMessageResponse> createOrGetDM(
        ...
        DirectMessageResponse dmResponse = directMessageService.createOrGetDirectMessage(currentUser.getId(), request.otherUserId());
    ...
    public ResponseEntity<List<DirectMessageConversationResponse>> getAllDMs(
            @AuthenticationPrincipal CustomUserDetails currentUser) {
        List<DirectMessageConversationResponse> conversations = directMessageService.getAllDirectMessages(currentUser.getId());
        return ResponseEntity.ok(conversations);
    }

    /**
     * Get DM conversation details. Only participants can access.
     * GET /v1/direct-messages/{dmId}
     *
     ...
     * GET /v1/direct-messages/{dmId}/messages?page={page}&size={size}
     *
     ...
     * POST /v1/direct-messages/{dmId}/messages
     *
     * @param dmId The ID of the direct message conversation.
    ...
    public ResponseEntity<DirectMessageMessageResponse> sendMessage(
            ...
            @AuthenticationPrincipal CustomUserDetails currentUser) {
        DirectMessageMessageResponse messageResponse = directMessageService.sendMessage(dmId, request, currentUser.getId());
        return new ResponseEntity<>(messageResponse, HttpStatus.CREATED);
    }

    /**
     * Edit message in DM. Only sender within 15 minutes.
     * PUT /v1/direct-messages/{dmId}/messages/{messageId}
     *
     * @param dmId The ID of the direct message conversation.
    ...
    public ResponseEntity<DirectMessageMessageResponse> editMessage(
            ...
            @AuthenticationPrincipal CustomUserDetails currentUser) {
        DirectMessageMessageResponse updatedMessage = directMessageService.editMessage(dmId, messageId, request, currentUser.getId());
        return ResponseEntity.ok(updatedMessage);
    }

    /**
     * Delete message in DM. Only sender.
     * DELETE /v1/direct-messages/{dmId}/messages/{messageId}
     *
     * @param dmId The ID of the direct message conversation.
