...
public record ChannelDetailResponse(
    ...
    public static ChannelDetailResponse from(Channel channel, String teamName, String projectName, List<ChannelMember> channelMembers) {
        ...
        if ("AD_HOC".equals(channelType)) {
            ...
            // For TEAM/PROJECT channels, members are derived, so we just provide a count
            // This count would typically come from a service layer query (e.g., teamService.countTeamMembers(teamId))
            // For now, we'll leave it null or 0, as the service layer will populate it.
            count = channelMembers.size(); // Placeholder, actual count from service
        }
